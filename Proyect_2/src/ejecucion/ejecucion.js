...
class Ejecucion {
    ...
    getDot() {
        this.contador = 0;
        this.dot = "digraph G {\n";
        this.dot += `overlap=false  label="by Kemel Ruano" fontsize=25;`;
        if (this.raiz != null) {
    ...
    generacionDot(nodo) {
        if (nodo instanceof Object) {
            ...
            this.dot += `node${idPadre}[label="${this.getStringValue(nodo.label)}" shape = "box"];\n`;
            if (nodo.hasOwnProperty("hijos")) {
                nodo.hijos.forEach((nodoHijo) => {
                    ...
                    if (nodoHijo instanceof Object) {
                        ...
                        this.dot += `node${idHijo}[label="${this.getStringValue(nodoHijo)}" shape = "doublecircle"];`;
                    }
                });
            }
        }
    ...
    recorrer(nodo) {
        ...
        if (this.soyNodo('INSTRUCCIONES', nodo)) {
            ...
            nodo.hijos.forEach((nodoHijo) => {
                if ( /* this.soyNodo('DECLARACION_VARIABLE', nodoHijo) ||  */this.soyNodo('DECLARACION_FUNCION_OR_METODO', nodoHijo) || this.soyNodo('DECLARACION_METODO', nodoHijo)) {
        ...
        // TOLOWER_OR_TOUPPER
        if (this.soyNodo('TOLOWER_OR_TOUPPER_OR_ROUND', nodo)) {
            const id = nodo.hijos[0];
            const exp = this.recorrer(nodo.hijos[5]);
            return { id, exp };
        }
        if (this.soyNodo('NATIVAS', nodo)) {
            const id = nodo.hijos[0];
            const exp = this.recorrer(nodo.hijos[5]);
            return { id, exp };
        }
        if (this.soyNodo('ASIGNACION_ARREGLOS', nodo)) {
            const id = nodo.hijos[0];
            const exp = this.recorrer(nodo.hijos[3]);
            return { id, exp };
        }
        if (this.soyNodo('ARREGLO_TIPO', nodo)) {
            const id = nodo.hijos[0];
        ...
        if (this.soyNodo('NEW_ARREGLO', nodo)) {
            const exp = this.recorrer(nodo.hijos[1]);
            return { exp };
        }
        if (this.soyNodo('PRIMERO_ARRAY', nodo)) {
            switch (nodo.hijos.length) {
                case 5: {
        ...
        // acesso vector
        if (this.soyNodo('ACCESSO_VECTOR', nodo)) {
            const id = nodo.hijos[0];
            switch (nodo.hijos.length) {
                case 5: {
                    const exp = this.recorrer(nodo.hijos[4]);
        ...
        // Pre-modificacion
        if (this.soyNodo('ASG_VECTOR', nodo)) {
            switch (nodo.hijos.length) {
                case 5: {
                    const exp = this.recorrer(nodo.hijos[1]);
                    return { exp };
        ...
        // MODICIAR VECTOR
        if (this.soyNodo('MODIFICAR_VECTOR', nodo)) {
            const exp = this.recorrer(nodo.hijos[0]);
        ...
        //DECLARACION_FUNCION
        if (this.soyNodo('DECLARACION_FUNCION_OR_METODO', nodo)) {
            switch (nodo.hijos.length) {
                case 9: {
                    const id = nodo.hijos[0];
                    // {tipo, type_generador?}
                ...
                case 10: {
                    if (this.soyNodo('LISTA_PARAMETROS', nodo.hijos[2])) {
                        const id = nodo.hijos[0];
                        //[Variable ...]
        ...
        //PUSH_ARREGLO
        if (this.soyNodo('PUSH_OR_POP_ARREGLO ', nodo)) {
            const id = nodo.hijos[0];
            switch (nodo.hijos.length) {
    ...
    puedoEjecutar(nodo) {
        ...
        if (this.soyNodo('INSTRUCCIONES', nodo)) {
            for (let nodoHijo of nodo.hijos) {
                ...
                if (this.soyNodo('DECLARACION_FUNCION_OR_METODO', nodoHijo)) {
            ...
            for (let nodoHijo of nodo.hijos) {
                ...
                if (this.soyNodo('INSTRUCCIONES', nodoHijo)) {
                    for (let nodoInst of nodoHijo.hijos) {
                        if (this.soyNodo('DECLARACION_FUNCION_OR_METODO', nodoInst)) {
                            return false;
                        }
                    }
                }
